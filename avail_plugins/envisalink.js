...
function Envisalink () {
  ...
  this.init = function() {
    if (nconf.get('envisalink:panelConfig')) {
      panelConfig = nconf.get('envisalink:panelConfig');
      // logger(panelConfig);
    }
  ...
  this.discover = function() {
    if (nconf.get('envisalink:panelConfig')) {
      notify(nconf.get('envisalink:panelConfig'));
  ...
  function updateZone(partitionNumber, zoneNumber, state) {
    ...
    logger(`update zone: ${zoneNumber} ${state}`);
    
    let zone = panelConfig.zones.find(x => x.zone == zoneNumber);
    // logger(zone);

    var msg = {type: 'zone', partition: partitionNumber, zone: zoneNumber, state: state, update: zone};
    // logger(msg);
    notify(msg);
  }
  ...
  function updatePartition(partitionNumber, state, alpha) {
    ...
    logger(`update panel: ${partitionNumber} ${state}`);
    let pc = panelConfig.partitions.find(x => x.partition == partitionNumber);
    // logger(pc);

    var msg = {type: 'partition', partition: partitionNumber, state: state, alpha: alpha, update: pc};
    // logger(msg);
    notify(msg);
  }
